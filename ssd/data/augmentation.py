...
class ToAbsoluteCoordsGray(object):
    def __call__(self, image, boxes=None, labels=None):
...
class RandomSaturation(object):
    '''
    Used for HSV channel pic, S channel random times 0.5 ~ 1.5
    H channel for Hue:colorPhase, [0, 360)
    S Saturation, [0%, 100%]
    V value: colorTune
    Need to clip? for S channel?
    '''
...
class Expand(object):
    #make border
    def __init__(self, mean, maxExpandRatio = 4):
        self.mean = mean
...
class PhotometricDistort(object):
    ...
    def __call__(self, image, boxes, labels):
        ...
        if random.randint(2):
            # first random contrast or last contrast
